...
const openai_1 = require("@lancedb/lancedb/embedding/openai");
class SearchService {
    storage;
    openaiApiKey;
    embeddingFunction = null;
    constructor(storage, openaiApiKey) {
        this.storage = storage;
        this.openaiApiKey = openaiApiKey;
        if (openaiApiKey) {
            this.embeddingFunction = new openai_1.OpenAIEmbeddingFunction({
                apiKey: openaiApiKey,
     ...
     * 混合搜索：同时进行全文搜索和语义搜索
     */
    async mixedSearch(query, filters) {
